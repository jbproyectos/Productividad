...
<body class="bg-gray-50 text-gray-900 font-sans">
                                        ...
                                        <td class="flex justify-center items-center px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <label class="inline-flex items-center cursor-pointer">
                                                <input type="checkbox" class="sr-only peer"
                                                    onchange="cambiarEstado(<?= $proyecto['id'] ?>, this.checked)"
                                                    <?= $proyecto['estatus_activo_donde'] == 'ACTIVO' ? 'checked' : '' ?>>
    ...
    <div id="modal-carga-masiva-dondes" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
        <div class="bg-white rounded-xl w-full max-w-md mx-auto">
            <div class="p-6">
                <h3 id="modal-titulo" class="text-lg font-bold text-gray-900 mb-4">Carga Masiva</h3>
                <form id="cargamasivadondes" class="p-2" method="post">
                    <div class="flex justify-between mb-1 text-gray-500 light:text-gray-400">
                        <div class="flex items-center justify-center w-full">
                            <label for="dropzone-file" id="file-dondes" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 light:hover:bg-bray-800 light:bg-gray-700 hover:bg-gray-100 light:border-gray-600 light:hover:border-gray-500 light:hover:bg-gray-600">
                                <div id="preview-dondes" class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <input id="dropzone-file" type="file" name="subeDondes" class="hidden" accept=".csv,.txt" />
                            </label>
                        </div>
                    </div>
                    <!-- Modal footer -->
    ...
    <script>
            ...
            let estado = checked ? 'ACTIVO' : 'INACTIVO';
                ...
                Swal.fire({
                    ...
                    if(result.isConfirmed){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "productividad/cambiar_estado/cambiar_estado_dondes.php",
                            data: { id: id, estatus_activo_donde: estado },
        ...
        function abrirModalCargaMasiva() {
            document.getElementById('modal-titulo').textContent = 'Carga Masiva de Donde';
            document.getElementById('btn-text').textContent = 'Subir Archivo';
            document.getElementById('modal-carga-masiva-dondes').classList.remove('hidden');
        }
        ...
        function cerrarModalCarMas() {
            document.getElementById('modal-carga-masiva-dondes').classList.add('hidden');
            // Limpiar parámetros de edición en la URL
            if (window.location.href.includes('accion=editar')) {
                window.location.href = window.location.pathname;
            }
        ...
        document.getElementById('modal-carga-masiva-dondes').addEventListener('click', function(e) {
            if (e.target === this) {
                cerrarModalCarMas();
            }
        });
    ...
    <script>
        $("#cargamasivadondes").submit(function(e) {
            e.preventDefault();
